# Release Process — GroupDocs.Parser.Mcp
...
1. **Bump the package version** in [build/dependencies.props](build/dependencies.props):
   ...
   ```xml
   <GroupDocsParserMcp>{NEW_VERSION}</GroupDocsParserMcp>
   ```

2. **Bump both `.mcp/server.json` versions** — [src/GroupDocs.Parser.Mcp/.mcp/server.json](src/GroupDocs.Parser.Mcp/.mcp/server.json) has **two** `version` fields that must both match:
...
3. **Update pinned versions in `README.md`** — search for the old version and replace every occurrence (`dnx GroupDocs.Parser.Mcp@…`, Claude Desktop config, VS Code `mcp.json`). Typically 4 places.
...
5. *(Optional, rare)* Bump external dependency versions in the same props file — `GroupDocsMcpCore`, `GroupDocsParser`, `MicrosoftExtensionsHosting`, `ModelContextProtocol`, `MicrosoftSourceLinkGithub`.

6. Commit + push:

   ```bash
   git add build/dependencies.props src/GroupDocs.Parser.Mcp/.mcp/server.json README.md changelog/NNN-*.md
...
dotnet test src/GroupDocs.Parser.Mcp.sln -c Release       # runs tests
docker build -f docker/Dockerfile -t parser-net-mcp:test .    # Dockerfile sanity check
```
...
4. Push `ghcr.io/groupdocs-parser/parser-net-mcp:{NEW_VERSION}` + `:latest`.
5. Push `docker.io/groupdocs/parser-net-mcp:{NEW_VERSION}` + `:latest`.

**`publish_prod.yml → publish_mcp_registry` job** (runs on every release):

1. Runs after `publish_prod` succeeds.
2. Uses GitHub OIDC to authenticate with the MCP Registry — namespace `io.github.groupdocs-parser/*` is auto-verified because the repo lives under `github.com/groupdocs-parser/*`.
3. Publishes `src/GroupDocs.Parser.Mcp/.mcp/server.json` to `https://registry.modelcontextprotocol.io`.
...
- [ ] `ghcr.io/groupdocs-parser/parser-net-mcp:{NEW_VERSION}` pullable
- [ ] `docker.io/groupdocs/parser-net-mcp:{NEW_VERSION}` pullable
- [ ] Smoke test from a clean machine: `dnx GroupDocs.Parser.Mcp@{NEW_VERSION} --yes`
- [ ] MCP Registry: `https://registry.modelcontextprotocol.io/v0/servers/io.github.groupdocs-parser%2Fgroupdocs-parser-mcp/versions/latest` returns 200 with `version` = `{NEW_VERSION}` and `_meta.io.modelcontextprotocol.registry/official.isLatest` = `true` (note: the slash in the server name must be URL-encoded as `%2F`, and the route requires the `/versions/latest` suffix — there is no bare `/v0/servers/{name}` endpoint)
...
| Secret | Purpose |
|---|---|
| `NUGET_API_KEY_PROD` | NuGet.org API key scoped to `GroupDocs.Parser.Mcp` |
...
| `DOCKERHUB_TOKEN` | Docker Hub access token scoped to `groupdocs/parser-net-mcp` |
...
The `publish_mcp_registry` job authenticates via GitHub OIDC. The Registry auto-verifies the `io.github.groupdocs-parser/*` namespace because the workflow runs inside `github.com/groupdocs-parser/*` — no DNS records, no manual tokens, no opt-in variable required. First-time publish claims the namespace automatically; subsequent releases update it.

If you ever fork the repo to a different org (e.g. `github.com/someone-else/*`), the Registry publish will fail until either the `server.json` `name` field is updated to match the new org's namespace or DNS verification is added — but for the canonical `groupdocs-parser` org, it just works.
