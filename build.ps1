...
Assert-ServerJsonVersionMatchesDependencies `
    ...
    -ServerJsonPath ".\src\GroupDocs.Parser.Mcp\.mcp\server.json" `
    -VersionPropertyName "GroupDocsParserMcp"

exec { & dotnet restore src\GroupDocs.Parser.Mcp.sln }

$isProd = $env:BUILD_TYPE -eq "PROD"

if ($isProd) {
    Write-Host "build: PROD build - stable version (no suffix)"
    exec { & dotnet build src\GroupDocs.Parser.Mcp.sln -c Release --verbosity quiet --nologo }
} else {
    $commitHash = $(git rev-parse --short HEAD)
    $buildSuffix = "local-$commitHash"
    Write-Host "build: DEV build - version suffix is $buildSuffix"
    exec { & dotnet build src\GroupDocs.Parser.Mcp.sln -c Release --version-suffix=$buildSuffix --verbosity quiet --nologo }
}
...
exec { & dotnet pack .\src\GroupDocs.Parser.Mcp\GroupDocs.Parser.Mcp.csproj @packArgs }
