...
[McpServerToolType]
public static class ExtractImagesTool
{
    [McpServerTool, Description(
        "Extracts all images from a document and saves them to storage as separate image files. " +
        "Supports PDF, DOCX, XLSX, PPTX, HTML, EPUB, and 30+ more formats with embedded images. " +
        ...
        "Returns a list of saved-path messages, one per extracted image (named '<basename>_image<N>.<ext>'). " +
        "On failure, the response text starts with 'Image extraction failed for' followed by the underlying exception type, message, and inner-exception chain.")]
    public static async Task<string> ExtractImages(
        ...
        [Description("Page number to extract images from (1-based). Omit for all pages.")] int? page = null)
    {
        ...
        {
            return ToolError.Format("Image extraction", resolved.FileName, ex, page.HasValue ? $" (page={page})" : null);
        }
    }
}
