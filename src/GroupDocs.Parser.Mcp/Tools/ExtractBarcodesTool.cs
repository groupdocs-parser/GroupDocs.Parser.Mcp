...
[McpServerToolType]
public static class ExtractBarcodesTool
{
    private static readonly JsonSerializerOptions JsonOptions = new() { WriteIndented = true };

    [McpServerTool, Description(
        "Extracts all barcodes and QR codes from a document and returns their decoded values and type names as JSON. " +
        "Detects Code128, QR Code, PDF417, DataMatrix, EAN-13, EAN-8, UPC, Aztec, and many more symbologies. " +
        "Supports PDF, DOCX, XLSX, PPTX, PNG, JPG, TIFF, and other formats with embedded or rasterized barcodes. " +
        ...
        "Returns a header line ('Found N barcode(s) in ...') followed by a JSON array of `{ index, value, type, page, confidence, angle }` per barcode, or a 'No barcodes found' message. " +
        "On failure, the response text starts with 'Barcode extraction failed for' followed by the underlying exception type, message, and inner-exception chain.")]
    public static async Task<string> ExtractBarcodes(
        ...
        [Description("Password for protected documents")] string? password = null)
    {
        ...
        {
            return ToolError.Format("Barcode extraction", resolved.FileName, ex, page.HasValue ? $" (page={page})" : null);
        }
    }
}
